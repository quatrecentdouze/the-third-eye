...
Collects CPU, memory, and system metrics — exposes them via Prometheus and a local UI.
...
- **Dashboard** — CPU, memory, uptime, health status, live sparklines
- **Logs** — real-time agent logs with search and filtering
- **Settings** — change collection interval and log level on the fly
- **Diagnostics** — export a full snapshot for troubleshooting
...
| `GET /api/status` | Health, metrics, config, build info |
